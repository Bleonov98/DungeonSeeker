...
void TextRenderer::RenderText(std::string text, glm::vec2 position, float scale, glm::vec3 colour)
{
    // activate corresponding render state	
    ResourceManager::GetShader("textShader").Use();
    ResourceManager::GetShader("textShader").SetVector3f("textColour", colour);
...
bool TextButton::TextCollision(float xMouse, float yMouse)
{
    glm::vec2 mousePos = glm::vec2(xMouse, yMouse);
    
    glm::vec2 maxPoint;
    maxPoint = position + size;

    return xMouse > position.x && xMouse < maxPoint.x && yMouse > position.y && yMouse < maxPoint.y;
}

void TextButton::RenderButton(float scale)
{
    size = glm::vec2(0.0f);
    float maxY = 0.0f;
